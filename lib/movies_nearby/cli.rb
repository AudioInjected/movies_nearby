class MoviesNearby::CLI
  ...
  def greeting
    ...
    puts "Please enter a number corresponding to the borough or type exit to exit (example: 1)"
    puts "1:The Bronx   2:Manhattan   3:Brooklyn  4: Staten-Island  5:Queens"  
    @borough = borough_info
    puts "Please enter your zip code (example: 10458)"
    @zip_code = zip_info
  end
 ...
 def start 
  ...
  def make_url
    BASE + @borough + "-ny/" + @zip_code + "/theaters/"
  end
  ...
  def create_theaters_movies
   theater_array = MoviesNearby::Scraper.scrape_theater(make_url)
  ...
  def list_theaters
    ...
    list_movie_info(theater)
  end
  
  def list_movie_info(theater)
    puts
